...
/**
 * Default specialist runner: uses Claude to imitate the sponsor product behavior
...
export function makeMockSpecialist(config: SpecialistConfig): SpecialistRunner {
  return {
    ...
    async bid(prompt, taskType): Promise<SpecialistDecision> {
      ...
      const data = await callClaudeJSON<BidLLMResponse>({
        systemPrompt,
        userPrompt,
        maxTokens: 256,
        timeoutMs: 10_000,
      ...
      if (
        ...
        // Coerce minimum viable bid from the cost baseline if Claude returned a malformed object
